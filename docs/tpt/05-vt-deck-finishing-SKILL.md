...
Katherine. Covers images, icons, beat legend, canonical slide order, appendix
...
- Pattern-break slides where the student cannot picture the organism (bee orchid,
  dodder, anglerfish). This is the highest-value placement in the deck: the beat
  fails if the student cannot see the anomalous case.
...
- Prefer unlabeled images. A caption that names the category hands the student the
  discrimination the beat is asking them to make.
...
- Choose an icon that names the beat's content, not a mood: mushroom on the
...
## 3. Beat numbers and the beat legend

**Decision (2026-08-06): remove the small grey corner beat numbers from all slides.**
They drift out of sequence the moment slides are reordered, and a bare integer tells
a student nothing.

Replace them with one **teacher-facing Beat legend slide** listing every slide in
the deck by **deck position** with the kind of beat it is. Position, not beat number,
because position is what the presenter shows.

**Critical constraint.** Do NOT put beat names in the corner of student slides.
...
excluded count is the one that must match the deck's real beat structure.

Beat vocabulary used on the legend: Bellringer · Orientation · Contrast pair ·
...
7. Beat legend (teacher navigation)
