...
- [ ] Confirm Composio is connected (kvond12). If a Drive/Sheets call errors, **retry first** — transient "upstream MCP server" blips are common — before reporting a disconnection.
- [ ] Default all Drive/Sheets/Docs through **Composio (kvond12)**, never the first-party Google Drive connector (it blocks AI-restricted school files).
...
- [ ] **Append** rows to the **Agency Watch** sheet → `Coding Master` tab (`18Cc-edFsozcn_04-1r1oUCouy6kFhUuFdeWOO3HWa3s`), `status = new`. Append-only — never overwrite (protects Katherine's verdicts). The `Before Class` tab surfaces them automatically for pre-class review.
